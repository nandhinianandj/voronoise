...
#' @export
grow_sapling <- function(startx=0, starty=0) {

  sapling <- tibble::tibble(
    x_0 = startx, y_0 = starty,  # first shoot starts at origin
    x_1 = startx, y_1 = .5 * starty, # first shoot guide is its midpoint
    x_2 = startx, y_2 = 1 * starty,  # first shoot grow to y = 1
...
grow_shoots <- function(time, shoots, param) {
  ...
  ch_seg_len <- dqsample::sample(x = param$scale, size = n_shoots, replace = TRUE)
  ch_seg_deg <- dqsample::sample(x = param$angle, size = n_shoots, replace = TRUE)
